...
AND	HOCPHAN.MAHOCPHAN = 'tindc'
--16
SELECT LOP.MAKHOA , KHOA.TENKHOA FROM KHOA , LOP
WHERE LOP.MAKHOA = KHOA.MAKHOA

--17
SELECT LOP.TENLOP , KHOA.TENKHOA , COUNT(DIEMTHI.MASV) FROM LOP , KHOA , DIEMTHI
WHERE DIEMTHI.LANTHI = 2
AND LOP.MAKHOA = KHOA.MAKHOA 
GROUP BY LOP.TENLOP , KHOA.TENKHOA , DIEMTHI.MASV
HAVING COUNT (DIEMTHI.MASV) > 3
--18
SELECT SINHVIEN.MASV , SINHVIEN.HODEM , SINHVIEN.TEN , SINHVIEN.NGAYSINH , AVG(DIEMTHI.DIEM) FROM SINHVIEN , DIEMTHI
WHERE SINHVIEN.MASV = DIEMTHI.MASV 
AND DIEMTHI.LANTHI =1 
GROUP BY SINHVIEN.MASV , SINHVIEN.HODEM , SINHVIEN.TEN , SINHVIEN.NGAYSINH
--19

SELECT SINHVIEN.MALOP , SINHVIEN.HODEM , SINHVIEN.TEN  FROM SINHVIEN , DIEMTHI
WHERE DIEMTHI.LANTHI = 2
AND SINHVIEN.MASV = DIEMTHI.MASV
AND(DIEMTHI.MAHOCPHAN = 'tinvp'
OR DIEMTHI.MAHOCPHAN = 'sql')
--20
SELECT DISTINCT HOCPHAN.TENHOCPHAN , COUNT(SINHVIEN.MASV) FROM HOCPHAN , SINHVIEN , DIEMTHI
WHERE SINHVIEN.MASV = DIEMTHI.MASV
AND DIEMTHI.MAHOCPHAN = HOCPHAN.MAHOCPHAN
AND DIEMTHI.LANTHI = 1
GROUP BY HOCPHAN.TENHOCPHAN , DIEMTHI.MAHOCPHAN 
--21
SELECT SINHVIEN.MASV , SINHVIEN.HODEM , SINHVIEN.TEN  , AVG(DIEMTHI.DIEM) FROM SINHVIEN , DIEMTHI
WHERE SINHVIEN.MASV = DIEMTHI.MASV 
AND DIEMTHI.LANTHI =1 
GROUP BY SINHVIEN.MASV , SINHVIEN.HODEM , SINHVIEN.TEN 
--22
SELECT LOP.MALOP , LOP.TENLOP ,COUNT(SINHVIEN.MASV) AS 'TỔng số sv' FROM SINHVIEN ,  LOP
WHERE SINHVIEN.MALOP = LOP.MALOP
GROUP BY LOP.MALOP , LOP.TENLOP 
--23
SELECT DISTINCT SINHVIEN.MASV , SINHVIEN.HODEM , SINHVIEN.TEN FROM SINHVIEN , DIEMTHI
WHERE SINHVIEN.MASV = DIEMTHi.MASV
AND DIEMTHI.LANTHI = 1
--24
--25
SELECT DISTINCT KHOA.TENKHOA , COUNT(SINHVIEN.MASV) FROM KHOA , SINHVIEN , DIEMTHI
WHERE SINHVIEN.MASV = DIEMTHI.MASV
AND DIEMTHI.LANTHI = 2
GROUP BY KHOA.TENKHOA , DIEMTHI.MASV
...
SELECT SUM(DIEMTHI.DIEM) AS 'tổng điểm thi sv(dl01-001)' FROM DIEMTHI
WHERE DIEMTHI.MASV = 'dl01-001'
AND DIEMTHI.LANTHI = 1

SELECT COUNT(DISTINCT DIEMTHI.MASV) AS 'Số sinh viên'
FROM DIEMTHI

SELECT AVG(DISTINCT DIEMTHI.DIEM) FROM DIEMTHI
WHERE DIEMTHI.MASV = 'dl01-002'
AND DIEMTHi.LANTHI = 1

SELECT DISTINCT COUNT(SINHVIEN.MASV) AS 'Số sinh viên' FROM SINHVIEN
WHERE SINHVIEN.MALOP ='dl01'

SELECT DISTINCT SINHVIEN.GIOITINH , COUNT(SINHVIEN.MASV) AS 'Số sinh viên' FROM SINHVIEN
